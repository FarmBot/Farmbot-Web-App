...
describe Api::DevicesController do
  ...
  describe "#seed" do
    ...
    def sequences_pick_from_seed_trough?(device)
      device.sequences.find_by(name: PublicSequenceNames::PICK_FROM_SEED_TROUGH)
    end

    def sequences_pick_from_seed_bin?(device)
      device.sequences.find_by(name: PublicSequenceNames::PICK_FROM_SEED_BIN)
    end
    ...
    it "seeds accounts with Genesis 1.2 data" do
      ...
      expect(sequences_pick_from_seed_trough?(device)).to be_kind_of(Sequence)
      expect(sequences_pick_from_seed_bin?(device)).to be_kind_of(Sequence)
    ...
    it "seeds accounts with Genesis 1.9 data" do
      start_tests "genesis_1.9"
      ...
      check_slot_pairing(tool_slots_slot_2?(device), "Seed Bin")
      check_slot_pairing(tool_slots_slot_3?(device), "Seed Tray")
      check_slot_pairing(tool_slots_slot_4?(device), "Soil Sensor")
      check_slot_pairing(tool_slots_slot_5?(device), "Rotary Tool")
      check_slot_pairing(tool_slots_slot_6?(device), "Seed Trough 1")
      check_slot_pairing(tool_slots_slot_7?(device), "Seed Trough 2")
    ...
    it "seeds accounts with Genesis XL 1.9 data" do
      start_tests "genesis_xl_1.9"
    ...
    it "seeds accounts with Express 1.0 data" do
      ...
      expect(sequences_pick_from_seed_trough?(device)).to_not be
      expect(sequences_pick_from_seed_bin?(device)).to_not be
      expect(settings_default_map_size_x?(device)).to eq(2900)
    ...
    it "seeds accounts when sequence versions not available:  Genesis XL 1.9" do
      start_tests "genesis_xl_1.9", false

      expect(sequences_mow_all_weeds?(device)).to be_kind_of(Sequence)
    end

    it "seeds accounts when sequence versions not available:  Genesis 1.9" do
      start_tests "genesis_1.9", false

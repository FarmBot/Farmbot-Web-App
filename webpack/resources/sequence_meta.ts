      ...
      switch (variableContents.kind) {
        ...
        case "tool":
          ...
          if (ts) {
            const { x, y, z } = ts.body;
            return { x, y, z };
          } // Not found? Fallthrough to 0,0,0
      }
    }
...
const determineDropdown = (): DropDownItem => {
  return { label: "FIXME", value: "0" };
};

const determineVariableValue = (): ValueOfVariable => {
  return { kind: "coordinate", args: { x: 0, y: 0, z: 0 } };
};

const determineEditable = (): boolean => {
  return true;
};
  ...
  const reducer: R = (acc, celeryNode) => {
    return ({
      ...
      [celeryNode.args.label]: {
        celeryNode,
        location: determineLocation(index, celeryNode),
        dropdown: determineDropdown(),
        editable: determineEditable(),
        variableValue: determineVariableValue(),
      }
    });
  };

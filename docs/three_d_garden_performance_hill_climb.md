...
| 222 | Precompute static plant-spread instance placement for active spread updates | Real `PlantSpreadInstances` click-to-add benchmark with 1,000 plants and 60 active-position updates over one second, sampled as 12 realistic drag interactions | 13.902 ms median frame batch; 16.065 ms median setup-plus-frame total; 31,000 median `getZ` calls | 8.688 ms median frame batch; 10.370 ms median setup-plus-frame total; 1,000 median `getZ` calls | 37.5% faster frames, saving 5.214 ms per 60-frame drag; 35.5% faster total interaction, saving 5.696 ms; 96.8% fewer `getZ` calls | Accepted; spread preview now precomputes each plant's static world position and soil height once, while active overlap color and scale still update per pointer movement | `Precompute spread placement for 37.5% faster frames` |
| 223 | Merge point pin and sphere marker geometry into one instanced marker mesh | Real `PointInstances` overlay benchmark with 1,000 radius points across 6 color buckets, sampled 20 times while measuring render setup and instanced-mesh draw-call proxies | 18 instanced meshes; 12 marker meshes; 6 radius-ring meshes; 1.519 ms median render setup | 12 instanced meshes; 6 marker meshes; 6 radius-ring meshes; 1.229 ms median render setup | 33.3% fewer instanced meshes overall, 50.0% fewer marker draw-call proxies, and 19.1% faster setup, saving 0.290 ms | Accepted; each point bucket now uses one shared merged marker geometry for the pin-plus-sphere shape, while radius rings and marker resolution stay unchanged | `Merge point markers for 33.3% fewer draw calls` |
| 224 | Avoid per-frame `moment()` day-start allocation in seasonal sun animation | Warm-cache `getAnimatedSeasonDate()` benchmark for `Summer`, matching one second of 60 animated frames and sampled 50 times | 0.234 ms median per 60-frame date lookup batch | 0.193 ms median per 60-frame date lookup batch | 17.3% faster, saving 0.040 ms per animated second | Rejected and rolled back; the percentage qualified, but the absolute saving is too small to matter in a real frame budget and does not justify touching the date default path | Not committed |
| 225 | Binary-search seasonal sun animation samples instead of scanning them linearly | Warm-cache `getAnimatedSeasonDate()` benchmark for `Summer`, matching one second of 60 animated frames and sampled 50 times | 0.228 ms median per 60-frame date lookup batch | 0.046 ms median per 60-frame date lookup batch | 80.0% faster, saving 0.183 ms per animated second for one caller | Rejected and rolled back; the percentage was strong, but the absolute saving remains sub-millisecond even across the realistic seasonal-animation callers and was not worth adding a separate search helper | Not committed |

...
--
-- Name: check_pointer_delete_integrity(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION check_pointer_delete_integrity() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN IF TG_TABLE_NAME = 'generic_pointers' AND EXISTS (SELECT id FROM generic_pointers WHERE pointer_type = 'GenericPointer' AND pointer_id = OLD.id) THEN RAISE EXCEPTION 'Polymorphic reference exists. There are records in generic_pointers that refer to the table % with id %. You must delete those records of table generic_pointers first.', TG_TABLE_NAME, OLD.id; RETURN NULL; ELSEIF TG_TABLE_NAME = 'plants' AND EXISTS (SELECT id FROM generic_pointers WHERE pointer_type = 'Plant' AND pointer_id = OLD.id) THEN RAISE EXCEPTION 'Polymorphic reference exists. There are records in generic_pointers that refer to the table % with id %. You must delete those records of table generic_pointers first.', TG_TABLE_NAME, OLD.id; RETURN NULL; ELSEIF TG_TABLE_NAME = 'tool_slots' AND EXISTS (SELECT id FROM generic_pointers WHERE pointer_type = 'ToolSlot' AND pointer_id = OLD.id) THEN RAISE EXCEPTION 'Polymorphic reference exists. There are records in generic_pointers that refer to the table % with id %. You must delete those records of table generic_pointers first.', TG_TABLE_NAME, OLD.id; RETURN NULL; ELSE RETURN OLD; END IF; END$$;


--
-- Name: check_pointer_upsert_integrity(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION check_pointer_upsert_integrity() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN IF NEW.pointer_type = 'GenericPointer' AND EXISTS (SELECT id FROM generic_pointers WHERE id = NEW.pointer_id) THEN RETURN NEW; ELSEIF NEW.pointer_type = 'Plant' AND EXISTS (SELECT id FROM plants WHERE id = NEW.pointer_id) THEN RETURN NEW; ELSEIF NEW.pointer_type = 'ToolSlot' AND EXISTS (SELECT id FROM tool_slots WHERE id = NEW.pointer_id) THEN RETURN NEW; ELSE RAISE EXCEPTION 'Polymorphic record not found. No % model with id %.', NEW.pointer_type, NEW.pointer_id; RETURN NULL; END IF; END$$;
...
--
-- Name: check_pointer_generic_pointers_delete_integrity_trigger; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER check_pointer_generic_pointers_delete_integrity_trigger BEFORE DELETE ON generic_pointers FOR EACH ROW EXECUTE PROCEDURE check_pointer_delete_integrity();


--
-- Name: check_pointer_plants_delete_integrity_trigger; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER check_pointer_plants_delete_integrity_trigger BEFORE DELETE ON plants FOR EACH ROW EXECUTE PROCEDURE check_pointer_delete_integrity();


--
-- Name: check_pointer_tool_slots_delete_integrity_trigger; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER check_pointer_tool_slots_delete_integrity_trigger BEFORE DELETE ON tool_slots FOR EACH ROW EXECUTE PROCEDURE check_pointer_delete_integrity();


--
-- Name: check_pointer_upsert_integrity_trigger; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER check_pointer_upsert_integrity_trigger BEFORE INSERT OR UPDATE ON generic_pointers FOR EACH ROW EXECUTE PROCEDURE check_pointer_upsert_integrity();

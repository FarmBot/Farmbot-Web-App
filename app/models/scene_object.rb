class SceneObject < ApplicationRecord
  TEXTURE_TYPES = %w(cloud grass wood soil aluminum concrete screen bricks water none).sort
  BAD_TEXTURE_TYPE = "%{value} is not valid. Valid options are: " +
                     TEXTURE_TYPES.map(&:inspect).join(", ")
  SHAPE_TYPES = %w(box cylinder sphere plant tray window laptop desk).sort
